setwd("D:/UTC/GI05/SY19/sy19_projet2/regression")
...
#too long to compute several times : stored here MSEs of GAM+SS with different numbers of principal components
...
for(k in 1 : Kfolds) {
  ...
  #VII.SVR
  #radial basis
  #optimal parameters given by tune() : epsilon = 0.2, cost=2.
...
boxplot(mse.regLin,mse.tree,mse.randomForest,mse.bagging,mse.glm,mse.lasso,mse.radial.svr,optimalSmooth[2,],names=c("regLin","decisionTree","rForest","Bagging","GLM","Lasso reg","SVR","GAM+SS"))
...
#estime MSE on random data :

folds <- sample(1:Kfolds, n, replace=TRUE)
estimate.MSE <- rep(0,Kfolds)
for(k in 1:Kfolds) {
  fit.radial.svr <- svm(y~., data=scaledDataDummyIrrWithoutYearAndDep[folds!=k,],epsilon=0.2,cost=2)
  pred.radial.svr = predict(fit.radial.svr, newdata=scaledDataDummyIrrWithoutYearAndDep[folds==k,-c(1)])
  estimate.MSE[k] <- mean((pred.radial.svr - scaledDataDummyIrrWithoutYearAndDep$y[folds==k])^2)
}
